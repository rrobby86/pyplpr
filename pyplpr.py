...
def provider_names_python(country=None):
    import locale
    if not country:
        country, _ = locale.getdefaultlocale()
    locale.setlocale(locale.LC_ALL, (country, "UTF-8"))
    months = tuple(locale.nl_langinfo(getattr(locale, "MON_" + str(i)))
            for i in range(1, 13))
    dows = tuple(locale.nl_langinfo(getattr(locale, "DAY_" + str(i%7+1)))
            for i in range(1, 8))
    return months, dows
...
if __name__ == "__main__":
    ...
    data = calendar_data(args.year)
    data["monthnames"], data["downames"] = provider_names_python(args.locale)
    if args.holidays:
